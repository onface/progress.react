...
/******/ 	__webpack_require__.p = "/progress.react/0.1.3";
/******/
    ...
    _createClass(ProgressGroup, [{
        ...
        value: function getFloatArray(numArray) {
            ...
            return numArray.map(function (item, i) {
                ...
                item = item < 0 ? 0 : item;
                var accur = Math.pow(10, accurArray[i]);
                if (/\./.test(item)) {
                    var decimal = String(item).replace(/^\d+\.(\d+)$/, '$1');
                    if (decimal.length < accurArray[i]) {
                        var fil = new Array(accurArray[i] - decimal.length).fill('0').join('');
...
var Progress = (_temp = _class = function (_Component) {
    ...
    _createClass(Progress, [{
        key: "getFloatNum",
        value: function getFloatNum(num) {
            var self = this;
            num = num > 100 ? 100 : num;
            num = num < 0 ? 0 : num;
            var accur = Math.pow(10, self.props.accuracy);
            if (/\./.test(num)) {
                var decimal = String(num).replace(/^\d+\.(\d+)$/, '$1');
                if (decimal.length < self.props.accuracy) {
                    var fil = new Array(self.props.accuracy - decimal.length).fill('0').join('');
                    num = String(num) + fil;
                } else {
    ...
    return Progress;
}(_react.Component), _class.ProgressGroup = __webpack_require__("./lib/ProgressGroup.js"), _temp);
